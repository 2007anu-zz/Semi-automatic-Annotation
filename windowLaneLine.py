...
"""
Annotating Window for LaneLine
"""
...
class LaneLineWindow(QtGui.QWidget):
	__version = '0.17'
	__release_date = '2017-03-30'
	...
	def __init__(self, *args):
		...
		self.setWindowTitle('DJI_NAVSegAnno - LaneLine')
		...
		self.mode_checkbox = QtGui.QCheckBox('Edit', self)
		self.mode_checkbox.setChecked(True)
		self.mode_checkbox.stateChanged.connect(self.mode_change)

		self.showSkip_checkbox = QtGui.QCheckBox('Show Skipped', self)
		...
		self.skip2Prev_button = QtGui.QPushButton(u'<--skip to prev (不标)', self)
		self.skip2Prev_button.clicked.connect(self.handle_skip2Prev_button)

		self.skip2Next_button = QtGui.QPushButton(u'(不标) skip to next-->', self)
		...
		self.tab_method_line = QtGui.QWidget()
		# self.tab_method_polygon = QtGui.QWidget()
		# self.tab_method_slider = QtGui.QWidget()
		self.tab_method_line2 = QtGui.QWidget()
		# self.tab_widget_method.addTab(self.tab_method_slider, 'Segmentation')
		# self.tab_widget_method.addTab(self.tab_method_polygon, 'Polygon')
		self.tab_widget_method.addTab(self.tab_method_line, u'脑补线 PolyLine')
		self.tab_widget_method.addTab(self.tab_method_line2, u'真实线 Binarized_Line')
		...
		# ## add buttons for drawing polygon
		# confirm_poly_button = QtGui.QPushButton('Finish Polygon', self)
		# confirm_poly_button.clicked.connect(self.f)
		...
		# deal with draw lines
		Vlayout_line = QtGui.QVBoxLayout(self.tab_method_line)
		self.add_line_setting(Vlayout_line)

		# deal with draw lines2
		Vlayout_line2 = QtGui.QVBoxLayout(self.tab_method_line2)
		self.add_line2_setting(Vlayout_line2)
		...
		# self.open_dir = '/home/sijin/Pictures/test_seg'
	...
	def add_line2_setting(self, layout):
		"""
		Use rightClick context menu to choose between:
		[GrabCut Select]
		[Undo GrabCut]
		[Polygon -]
		[Polygon +]
		"""
		print "cur_line_type: ", self.label_seg.cur_line_type2

		for i, name in enumerate(self.label_seg.line_types.keys()):
			# if name == "polygonPlus" or name == "polygonMinus" or name == "polygonOtsu" or name == "default":
			if name[:3] == "fnc":
				continue
			## set color_bar dispaly next to radio_btn
			b_label = QtGui.QLabel()
			color = self.label_seg.line_types[name]["color_fill"][0]
			color_square = QtGui.QPixmap(QtCore.QSize(7, 7))
			color_square.fill(QtGui.QColor(color[0], color[1], color[2], 255))
			b_label.setPixmap(color_square)
			b = QtGui.QRadioButton(
				unicode(
					name +
					" " +
					self.label_seg.line_types[name]["chinese"] +
					" | " +
					str(self.label_seg.line_types[name]["label"])))
			if (i == 0):
				b.setChecked(True)
			b.toggled.connect(functools.partial(self.line_type_changed, name))

			Hlayout = QtGui.QHBoxLayout(self.tab_method_line2)
			Hlayout.addWidget(b)
			Hlayout.addWidget(b_label)
			layout.addLayout(Hlayout)
			layout.addLayout(Hlayout)
			###  
			self.radiobtn_dict2[name] = b
		...
		b = QtGui.QRadioButton(
				unicode(
					lastName +
					" 所有类别"))
		b.toggled.connect(functools.partial(self.line_type_changed, lastName))
		layout.addWidget(b)  # add the RadioButton on layout in 1 col
		self.radiobtn_dict2[lastName] = b

		#### create spinBox for choosing ID for line_Seg just created
		sp_label = QtGui.QLabel()
		sp_label.setText("LaneLine ID: ") 
		self.sp = QtGui.QSpinBox(self)
		self.sp.setRange(-128, 127)
		self.sp.valueChanged.connect(self.spinBoxValueChange_handel)
		Hlayout = QtGui.QHBoxLayout(self.tab_method_line2)
		Hlayout.addWidget(sp_label)
		Hlayout.addWidget(self.sp)
		layout.addLayout(Hlayout)

		# Delete Selected Line
		btn = QtGui.QPushButton('Delete all LaneLine Seg', self)
		btn.clicked.connect(self.handle_delete_all_LaneLine_Seg_btn)
		layout.addWidget(btn)


	def line_type_changed(self, selected_line_type):
		# handler of RatioButton for selecting line_type
		## cur_line_type for <line> mode
		if (self.edit_method == 'line'):
			# 1. Set cur_line_type
			self.label_seg.cur_line_type = selected_line_type
			logging.warn(
					'<{}>: Change cur line type to: {}\n'.format(self.edit_method, self.label_seg.cur_line_type))
			print '<{}>: Change cur line type to: {}\n'.format(self.edit_method, self.label_seg.cur_line_type)

			# # 2. check the corresponding radioBtn
			# self.radiobtn_dict[selected_line_type].setChecked(True)
	
			# 3. If selected line, change line_label of the selected line
			if (self.label_seg.selected_line is not None):
				logging.warn(
					'Change line label of selected line to: {}\n'.format(selected_line_type))
				self.label_seg.selected_line.line_label = selected_line_type
				self.label_seg.selected_line_disp(True)
				self.label_seg.update()

		## cur_line_type for <line2> mode
		elif (self.edit_method == 'line2'):
			self.label_seg.cur_line_type2 = selected_line_type
			if selected_line_type == "all":
				self.label_seg.cur_line_label = 255
				self.mode = 'view'  ## unable to edit
				logging.warn("In [{}] mode, unable to edit!!!".format(self.mode))
			else:
				self.mode = 'edit'  ## able to edit
				self.label_seg.collect_poly_points = []
				self.label_seg.zoomed_collect_points = []
				self.label_seg.cur_line_label = self.label_seg.line_types[self.label_seg.cur_line_type2]['label'] 
				logging.warn(
					'<{}>: Change cur line2 type to: {}\n'.format(self.edit_method, self.label_seg.cur_line_type2))
				print '<{}>: Change cur line2 type to: {}\n'.format(self.edit_method, self.label_seg.cur_line_type2)

			# self.label_seg.update_disp()
			self.label_seg.update()


	def handle_delete_selected_line_button(self):
		# handler for deleting selected line (name), redisplay label_seg img
		print "Deleting selected line !!!"
		if (self.label_seg.selected_line is not None):
			self.label_seg.line_set.remove(self.label_seg.selected_line)
			self.label_seg.collect_poly_points = []
			self.label_seg.break_idx = []
			# self.label_seg.clear_boundary_info()
			self.label_seg.selected_line = None
			logging.warn('Selected line deleted!!!')

		# print ">>>>>>>>>>line_set1 --> {}".format(self.label_seg.line_set)
		self.label_seg.update_disp()
		self.label_seg.update()
	...
	def tab_method_onchange(self, idx):
		...
		if idx == 0:
			self.edit_method = 'line'
			logging.warn('Changed to use <Line>\n')
		elif idx == 1:
			self.edit_method = 'line2'
			self.line_type_list.append("all")
			self.line2_mode = "polygonOtsu"  # default line2_mode
			self.polygonOtsuAction.setChecked(True)
	...
	# 		# ??? Why not self.label_seg.update_disp() ???
	# 		logging.warn('Changed to use <superpixel>\n')
	# 	elif idx == 1:
	# 		self.edit_method = 'polygon'
	# 		self.label_seg.update_disp()
	# 		self.label_seg.update()
	# 		logging.warn('Changed to use <Polygon>\n')
	# 	elif idx == 2:
	# 		self.edit_method = 'line'
	# 		self.label_seg.update_disp()
	# 		self.label_seg.update()
	...
	def mode_change(self, state):
		if state == QtCore.Qt.Checked:
			self.mode = 'edit'
			logging.warn('Enter ---[Edit] mode\n')
		else:
			self.mode = 'view'
			logging.warn('Enter ---[view] mode\n')

	def showSkip_change(self, state):
		if state == QtCore.Qt.Checked:
			self.ShowSkipImg = True
			logging.warn('[Show Skipped Images]\n')
		else:
			self.ShowSkipImg = False
			logging.warn('[Hide Skipped Images]\n')		
	...
	def handle_delete_button(self):
		...
		win_text = "Delete anno. data"
		questiong_msg = "Are you sure to delete ALL segmentation annotation on this image?"
		choice = QtGui.QMessageBox.question(self, win_text, questiong_msg, QtGui.QMessageBox.Yes, QtGui.QMessageBox.No)
	...
	def process_anno(self, toNext=True):
		...
		# if img.size[1] > 720:
		# 	H = 720
		# 	W = int(img.size[0] * (720.0/img.size[1]))
		# 	img = img.resize((W, H))
		...
		self.label_seg.set_ref_img(img_arr)
		# ### TEST
		...
		print "line_type_list: ", self.line_type_list
	...
	def handle_open_button(self):
		...
		self.init_images_path()
		if self.open_dir:
			self.anno_dir = self.open_dir + '_anno'
	...
	def keyReleaseEvent(self, event):
		...
		if event.key() == QtCore.Qt.Key_Z or event.key() == QtCore.Qt.Key_W:
		...
		if (event.key() == QtCore.Qt.Key_B or event.key() == QtCore.Qt.Key_N or event.key() == QtCore.Qt.Key_M) and self.edit_method == "line2":
	...
	def keyPressEvent(self, event):
		...
		elif event.key() == QtCore.Qt.Key_B and self.edit_method == "line2":
			if event.isAutoRepeat():
				return
			self.label_seg.disp_segBI_on()

		elif event.key() == QtCore.Qt.Key_N and self.edit_method == "line2":
			if event.isAutoRepeat():
				return
